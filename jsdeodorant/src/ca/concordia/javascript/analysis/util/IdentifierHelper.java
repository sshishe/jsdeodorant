...
public class IdentifierHelper {
	private static final Logger log = Logger.getLogger(IdentifierHelper.class
			.getName());
	...
	public static AbstractIdentifier getIdentifier(ParseTree node,
		...
		if (node instanceof LiteralExpressionTree) {
			...
			return new CompositeIdentifier(node.asNewExpression().operand,
					identifier);
			...
			return getIdentifier(node.asMemberExpression().operand,
					new CompositeIdentifier(node.asMemberExpression().operand,
							identifier));
			...
			return getIdentifier(
					node.asMemberLookupExpression().operand,
					new CompositeIdentifier(
							node.asMemberLookupExpression().operand, identifier));
		} else if (node instanceof ParenExpressionTree) {
			identifier = new PlainIdentifier(node);
			return new CompositeIdentifier(node.asParenExpression().expression,
					identifier);
			...
			return new CompositeIdentifier(node.asCallExpression().operand,
					identifier);
		} else if (node instanceof FunctionDeclarationTree) {
			// if (node.asFunctionDeclaration().name != null)
	...
	public static AbstractIdentifier findLValue(AbstractStatement statement,
			ParseTree rightValue) {
		if (statement.getStatement() instanceof ExpressionStatementTree) {
			...
			if (epxressionStatement.expression instanceof BinaryOperatorTree) {
				return IdentifierHelper
						.getIdentifier(epxressionStatement.expression
								.asBinaryOperator().left);
			}
		} else if (statement.getStatement() instanceof VariableStatementTree) {
			VariableStatementTree variableStatement = statement.getStatement()
					.asVariableStatement();
			...
			for (VariableDeclarationTree variableDeclaration : variableDeclarationListTree.declarations)
				if (variableDeclaration.initializer != null
						&& variableDeclaration.initializer.equals(rightValue))
					return IdentifierHelper
							.getIdentifier(variableDeclaration.lvalue);
		}
		return null;
	}
}
