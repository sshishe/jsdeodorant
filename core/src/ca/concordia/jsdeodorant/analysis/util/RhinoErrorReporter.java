...
public class RhinoErrorReporter {
	...
	static final DiagnosticType TYPE_PARSE_ERROR = DiagnosticType.warning(
			"JSC_TYPE_PARSE_ERROR", "{0}");

	// Special-cased errors, so that they can be configured via the
	// warnings API.
	static final DiagnosticType TRAILING_COMMA = DiagnosticType.error(
			"JSC_TRAILING_COMMA",
	...
	static final DiagnosticType DUPLICATE_PARAM = DiagnosticType.error(
			"JSC_DUPLICATE_PARAM", "Parse error. {0}");

	static final DiagnosticType INVALID_PARAM = DiagnosticType.warning(
			"JSC_INVALID_PARAM", "Parse error. {0}");

	static final DiagnosticType BAD_JSDOC_ANNOTATION = DiagnosticType.warning(
			"JSC_BAD_JSDOC_ANNOTATION", "Parse error. {0}");

	static final DiagnosticType JSDOC_IN_BLOCK_COMMENT = DiagnosticType
			.warning("JSC_JSDOC_IN_BLOCK_COMMENT", "Parse error. {0}");

	static final DiagnosticType MISPLACED_TYPE_ANNOTATION = DiagnosticType
	...
	static final DiagnosticType PARSE_TREE_TOO_DEEP = DiagnosticType.error(
			"PARSE_TREE_TOO_DEEP", "Parse tree too deep.");

	static final DiagnosticType INVALID_OCTAL_LITERAL = DiagnosticType.warning(
			"INVALID_OCTAL_LITERAL",
			"This style of octal literal is not supported in strict mode.");

	static final DiagnosticType ES6_FEATURE = DiagnosticType.error(
			"ES6_FEATURE",
	 ...
	 * holder {0} with a wild card that matches all possible strings. Also put
	 * the any non-place-holder in quotes for regex matching later.
	 */
	...
	private RhinoErrorReporter(AbstractCompiler compiler) {
		...
		typeMap = ImmutableMap
				...
				// Trailing comma
				.put(replacePlaceHolders("Trailing comma is not legal in an ECMA-262 object initializer"),
						TRAILING_COMMA)

				// Duplicate parameter
				.put(replacePlaceHolders("Duplicate parameter name \"{0}\""),
						DUPLICATE_PARAM)

				.put(Pattern.compile("^invalid param name.*"), INVALID_PARAM)

				// Unknown @annotations.
				.put(replacePlaceHolders(SimpleErrorReporter
						.getMessage0("msg.bad.jsdoc.tag")),
						BAD_JSDOC_ANNOTATION)

				.put(Pattern.compile("^\\QNon-JSDoc comment has annotations. "
						+ "Did you mean to start it with '/**'?\\E"),
						JSDOC_IN_BLOCK_COMMENT)

				// Unexpected @type annotations
				.put(Pattern
						.compile("^Type annotations are not allowed here.*"),
				...
				// Parse tree too deep.
				.put(replacePlaceHolders("Too deep recursion while parsing"),
						PARSE_TREE_TOO_DEEP)
				...
				.put(Pattern
						.compile("^this language feature is only supported in es6 mode.*"),
						ES6_FEATURE)
